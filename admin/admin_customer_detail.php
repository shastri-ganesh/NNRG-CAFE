...
?>
    ...
    <title>Customer Profile | NNRG-CÁFE</title>
</head>
...
?>
            ...
            <div class="row row-cols-1 notibar">
                <div class="col mt-2 ms-2 p-2 bg-danger text-white rounded text-start">
                    <i class="bi bi-x-circle ms-2"></i><span class="ms-2 mt-2">Failed to update customer
                        password.</span>
                </div>
            </div>
...
?>
        ...
        <?php
// Select customer record from database securely
$cid = isset($_GET["c_id"]) ? $_GET["c_id"] : 0;

// Cast to an integer to prevent invalid string SQL crashes like the <br> tag logged
$cid = intval($cid);

$query = "SELECT c_username,c_firstname,c_lastname,c_email,c_gender,c_type FROM customer WHERE c_id = ? LIMIT 1";
...
if ($result->num_rows > 0) {
    ...
    // Default empty values to prevent crashes below
    $row = ["c_username" => "Unknown", "c_firstname" => "Unknown", "c_lastname" => "", "c_gender" => "", "c_type" => "", "c_email" => ""];
}
$stmt->close();
?>
