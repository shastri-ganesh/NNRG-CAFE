...
// Block normal Admins from editing independent shops (like Bakery s_id=11)
$target_s_id = isset($_POST["s_id"]) ? $_POST["s_id"] : (isset($_GET["s_id"]) ? $_GET["s_id"] : 0);
if ($_SESSION["utype"] == "ADMIN" && $target_s_id > 10) {
    header("location: ../restricted.php");
    exit(1);
}
...
?>
        <form method="POST" action="admin_shop_edit.php?s_id=<?php echo $s_id; ?>" class="form-floating"
            enctype="multipart/form-data">
            <h2 class="mt-4 mb-3 fw-normal text-bold"><i class="bi bi-pencil-square me-2"></i>Update Shop Information
            </h2>
            ...
            <div class="form-floating mb-2">
                <input type="email" class="form-control" id="email" placeholder="E-mail" name="s_email"
                    value="<?php echo $row["s_email"]; ?>" required>
                <label for="email">E-mail</label>
            </div>
            <div class="form-floating mb-2">
                <input type="text" class="form-control" id="shoplocation" placeholder="Location"
                    value="<?php echo $row["s_location"]; ?>" name="s_location" required>
                <label for="shoplocation">Shop Location</label>
            </div>
            <div class="form-floating mb-2">
                <input type="text" class="form-control" id="shopphoneno" placeholder="Phone Number"
                    value="<?php echo $row["s_phoneno"]; ?>" name="s_phoneno" required>
