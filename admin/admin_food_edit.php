...
if (isset($_POST["upd_confirm"])) {
    ...
    // Security check: SHOP_ADMIN can only update their own shop's items
    if (isset($_SESSION['utype']) && $_SESSION['utype'] === 'SHOP_ADMIN') {
        if ($s_id != $_SESSION['shop_id']) {
            header("location: ../restricted.php");
            exit(1);
        }
    }
        ...
        <form method="POST" action="admin_food_edit.php" class="form-floating" enctype="multipart/form-data">
            ...
            <div class="form-floating mb-2">
                <input type="number" step=".25" min="0.00" max="999.75" class="form-control" id="f_price"
                    placeholder="Price (INR)" value="<?php echo $row["f_price"]; ?>" name="f_price" required>
